...
class IVAnalyzer:
    ...
    def clean_data(self, filepath: str) -> pd.DataFrame:
        """
        Clean and load data from file - replace this with your actual data cleaning logic
        ...
        pd.DataFrame
            Cleaned dataframe
        """
        # This is a placeholder - replace with your actual data cleaning logic
        # For now, assume it's a CSV or tab-delimited file
        try:
            ...
            # If all delimiters fail, try reading as whitespace-separated
    ...
    def load_and_process_data(self, filepath: str, data_type: str = 'metal') -> pd.DataFrame:
        ...
        try:
            ...
            # Check for required columns - modify these based on your actual column names
            required_columns = ['i-v_current_ch1_ma', 'temperature_k', 'i-v_voltage_ch1_v']
            missing_columns = [col for col in required_columns if col not in df.columns]
            
            if missing_columns:
                print(f"Missing columns: {missing_columns}")
                print("Please check your data file and update column names accordingly")
                return df  # Return as-is for inspection
    ...
    def calculate_conductivity_slopes(self, df: pd.DataFrame) -> Dict[float, Dict]:
        ...
        required_columns = ['temperature_group', 'i-v_voltage_ch1_v', 'current_density']
        missing_columns = [col for col in required_columns if col not in df.columns]
        
        if missing_columns:
            raise ValueError(f"Missing required columns for slope calculation: {missing_columns}")
    ...
    def create_iv_plots(self, df: pd.DataFrame, save_path: Optional[str] = None, 
        ...
        if missing_columns:
            print(f"Cannot create plots: Missing required columns: {missing_columns}")
            return
        ...
        for temp, group in df.groupby('temperature_group'):
            ...
            # Add legend only if we have something to show
            if has_legend:
                ax.legend()
    ...
    def analyze_complete_dataset(self, filepath: str, save_path: Optional[str] = None) -> Dict:
        ...
        if 'current_density' not in df.columns:
            print("Warning: Data processing incomplete. Check your data file format and column names.")
            return {'data': df, 'slopes': {}, 'report': 'Analysis incomplete due to data issues'}
